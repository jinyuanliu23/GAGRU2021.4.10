...
class LayerParams:
    ...
    def get_weights(self, shape):
        if shape not in self._params_dict:
            nn_param = torch.nn.Parameter(torch.empty(*shape, device=device1))
    ...
    def get_biases(self, length, bias_start=0.0):
        if length not in self._biases_dict:
            biases = torch.nn.Parameter(torch.empty(length, device=device1))
...
class GAGRUCell(torch.nn.Module):
    def __init__(self, num_units, adj_mx,  num_nodes,input_dim, nonlinearity='tanh', use_ga_for_ru=True,**model_kwargs):
        ...
        self.linear111 = torch.nn.Linear(self.input_dim *2,1)
        self.linear222 = torch.nn.Linear(self.input_dim *2,1)
